...
async function readApiResponse(res){const type=res.headers.get('content-type')||'';if(type.includes('application/json'))return await readApiResponse(res);const text=await res.text();if(text.includes('<html')||text.includes('<!DOCTYPE'))throw new Error('The server returned an HTML error page instead of JSON. Check the Render deployment/logs.');throw new Error(text||`Server returned HTTP ${res.status}`)}

...
async function readApiResponse(res){const type=(res.headers.get('content-type')||'').toLowerCase();if(type.includes('application/json')){try{return await res.json()}catch(_){throw new Error(`The server returned invalid JSON (HTTP ${res.status}). Check the Render deployment/logs.`)}}const text=await res.text();if(/<html|<!doctype/i.test(text))throw new Error('The server returned an HTML error page instead of JSON. Check the Render deployment/logs.');throw new Error(text.trim()||`Server returned HTTP ${res.status}`)}
...
function selectFile(f){if(!f)return;if(!['image/jpeg','image/png','application/pdf'].includes(f.type)){alert('Choose JPG, JPEG, PNG or PDF.');return}selectedFile=f;$('process').disabled=false;$('fileInfo').textContent=`Selected: ${f.name} · ${(f.size/1024/1024).toFixed(2)} MB`;$('preview').style.display='none';if(f.type==='application/pdf'){$('fileInfo').textContent+=` · PDF will be rendered page-by-page on the server.`;return}const r=new FileReader();r.onload=e=>{$('preview').src=e.target.result;$('preview').style.display='block'};r.readAsDataURL(f);}
...
async function loadReviewQueue(){
  ...
  try{const r=await fetch(API+'/api/review-queue?status=PENDING&_='+Date.now(),{headers:{'X-Auth-Token':authToken,'Cache-Control':'no-cache'},cache:'no-store'});const d=await readApiResponse(r);if(!r.ok||d.status!=='success')throw new Error(d.message||'Review queue lookup failed');
...
async function openReviewCase(caseId){
  ...
  try{const r=await fetch(API+'/api/review-queue/'+encodeURIComponent(caseId),{headers:{'X-Auth-Token':authToken}});const d=await readApiResponse(r);if(!r.ok||d.status!=='success')throw new Error(d.message||'Could not load case');const c=d.case||{}, res=c.result||{}, f=res.fields||{}, citizenData=c.citizen||{};
...
async function calculateCalibrationMetrics(){
  ...
  const r=await fetch(API+'/api/metrics',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({confidences:conf,correctness:corr})}); const d=await readApiResponse(r);
...
async function loadLRMS(){
  ...
  try{
    ...
    const d=await readApiResponse(r); if(!r.ok||d.status!=='success')throw new Error(d.message||'LRMS lookup failed');
...
async function loadGIS(){
  ...
  try{
    const r=await fetch(API+'/api/gis/parcel?survey_number='+encodeURIComponent(survey)+'&_='+Date.now(),{headers:{'X-Auth-Token':authToken,'Cache-Control':'no-cache'},cache:'no-store'});const d=await readApiResponse(r);
...
async function runDuplicateCheck(){
  ...
  try{
    const r=await fetch(API+'/api/duplicate-check',{method:'POST',headers:{'X-Auth-Token':authToken},body:fd});const d=await readApiResponse(r);if(!r.ok||d.status!=='success')throw new Error(d.message||'Duplicate check failed');
    let html='';
